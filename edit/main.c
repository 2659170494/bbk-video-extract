...
int read_header(FILE *f){
   ...
   fprintf(stderr,"Start Read Header! \n");
...
int main(int argc,char *argv[]){
   ...
   fprintf(stderr,"File Load! \n");
   if(read_header(f)){
      fprintf(stderr,"read_header OK! \n");
      char ofname[0x400];
      fprintf(stderr,"ofname is %s \n",ofname);
      strcpy(ofname,fname);
      fprintf(stderr,"fname is %s \n",fname);
      fprintf(stderr,"strcpy ofname OK! \n");
      char *ibuf;
      char *obuf;
      ibuf=header_buf+(lrc_offs-9);
      fprintf(stderr,"ibuf is %s \n",ibuf);
      while(*ibuf==' '){
	 ibuf--;
      }
      *(ibuf+1)=0;
      fprintf(stderr,"ibuf OK! \n");
      size_t ol=0x200,il=(ibuf-(char *)header_buf)-0x16;
      fprintf(stderr,"il guess! \n");
      if(il==lrc_offs-0x1F){
         fprintf(stderr,"il continue! \n");
         fprintf(stderr,"ofname is %s \n",ofname);
	 obuf=strrchr(ofname,'.');
    fprintf(stderr,"oubf is %s \n",obuf);
	 char *p=obuf-1;
    fprintf(stderr,"char *p OK! \n");
	 obuf+=5;
	 *obuf=0;
    fprintf(stderr,"*obuf OK! \n");
    int yoshi = 1;
    fprintf(stderr,"*p num is %d \n",*p!='/');
	 while(*p!='/'){
      fprintf(stderr,"*p! OK!  %d\n",yoshi);
      yoshi = yoshi + 1;
	    *(p+5)=*p;
       fprintf(stderr,"%d \n",*(p+5));
	    p--;
    ...
    fprintf(stderr,"il continue while OK! \n");
	 p[1]='d';
	 p[2]='o';
	 p[3]='n';
	 p[4]='e';
	 p[5]='-';
    fprintf(stderr,"p list 'done-' OK! \n");
	 
      }
      else{
         fprintf(stderr,"il else \n");
